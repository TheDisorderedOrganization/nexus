...
class LargestClusterSizeAnalyzer(BaseAnalyzer):
    def __init__(self, settings: Settings) -> None:
        ...
        self.largest_cluster_sizes: Dict[str, List[float]] = {}
    ...
    def analyze(self, frame: Frame) -> None:
        ...
        for connectivity in connectivities:
            ...
            if sizes:
                ...
                if connectivity not in self.largest_cluster_sizes:
                    self.largest_cluster_sizes[connectivity] = []
                self.largest_cluster_sizes[connectivity].append(max_size)
                ...
                if connectivity not in self.largest_cluster_sizes:
                    self.largest_cluster_sizes[connectivity] = []
                self.largest_cluster_sizes[connectivity].append(0.0)
    ...
    def finalize(self) -> Dict[str, float]:
        for connectivity, sizes in self.largest_cluster_sizes.items():
        ...
        return {"concentrations": self.concentrations, "largest_cluster_size": self.largest_cluster_sizes, "std": self.std}
