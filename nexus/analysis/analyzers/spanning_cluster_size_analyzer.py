...
class SpanningClusterSizeAnalyzer(BaseAnalyzer):
    ...
    def analyze(self, frame: Frame) -> None:
        ...
        for connectivity in connectivities:
            ...
            if connectivity not in self.spanning_cluster_size:
                self.spanning_cluster_size[connectivity] = []
            self.spanning_cluster_size[connectivity].append(max_size)

        self.update_frame_processed(frame)

    def finalize(self) -> None:
        for connectivity, sizes in self.spanning_cluster_size.items():
            self.spanning_cluster_size[connectivity] = np.mean(sizes)
            self.std[connectivity] = np.std(sizes, ddof=1)

    def get_result(self) -> Dict[str, float]:
        return {"spanning_cluster_size": self.spanning_cluster_size, "std": self.std}
