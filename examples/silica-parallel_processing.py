...
def process_trajectory(trajectory, output, pressure):
    ...
    settings.export_directory.set_value(f"./export/silica-parallel_processing")
...
settings.print_clusters_positions.set_value(
    False
)  # Set to True to print cluster coordinates to XYZ files (/!\ careful for storage consumption)
...
with ProcessPoolExecutor(max_workers=8) as executor:
    ...
    for i, trajectory in progress_bar:
        ...
        progress_bar.set_description(
            f"Processing ... \u279c {str(trajectory).split('/')[-1]}"
        )
